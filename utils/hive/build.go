...
func BuildToInsertStream(table string, model interface{}, versionIndex int, orUpdate bool, options ...*Schema) string {
	modelType := reflect.TypeOf(model)
	var cols []*FieldDB
	if len(options) > 0 && options[0] != nil {
		cols = options[0].Columns
	...
	return fmt.Sprintf("insert into %v (%v) values ", table, strings.Join(icols, ","))
}

func BuildToInsertStreamValues(table string, model interface{}, versionIndex int, orUpdate bool, options ...*Schema) string {
	modelType := reflect.TypeOf(model)
	var cols []*FieldDB
	if len(options) > 0 && options[0] != nil {
		cols = options[0].Columns
